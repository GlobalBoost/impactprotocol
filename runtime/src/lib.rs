...
//! The Impact Protocol runtime. This can be compiled with `#[no_std]`, ready for Wasm.
...
use frame_support::{
	...
	parameter_types,
	traits::{
		AsEnsureOriginWithArg, ConstBool, ConstU16, ConstU32, ConstU128, FindAuthor, Currency, EitherOfDiverse,
...
pub use impact_primitives::{AccountId, Signature};
use impact_primitives::{AccountIndex, Balance, BlockNumber, Hash, Index, Moment,currency::*,time::*};
...
use sp_runtime::{
	...
	traits::{
		...
		SaturatedConversion, StaticLookup, DispatchInfoOf, Dispatchable,
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
	spec_name: create_runtime_str!("impact-runtime"),
	impl_name: create_runtime_str!("impact-runtime"),
...
parameter_types! {
	...
	pub const MaxAuthorities: u32 = 1000_000;
	...
	Inner: FindAuthor<AccountId>,
{
	...
	{
		if let Some(accid) = Inner::find_author(digests)
		{
			return Some(H160::from_slice(&accid.to_raw_vec()[0..20]));
		}
		None
...
impl pallet_evm::Config for Runtime {
	...
	type OnCreate = ();
    type FindAuthor = FindAuthorAdapter<Rewards>;
...
construct_runtime!(
	pub enum Runtime where
		Block = Block,
		NodeBlock = impact_primitives::Block,
...
impl_runtime_apis! {
	...
	impl sp_consensus_pow::DifficultyApi<Block, impact_primitives::Difficulty> for Runtime {
		fn difficulty() -> impact_primitives::Difficulty {
			difficulty::Pallet::<Runtime>::difficulty()
		}
	}

	impl impact_primitives::AlgorithmApi<Block> for Runtime {
		fn identifier() -> [u8; 8] {
			impact_primitives::ALGORITHM_IDENTIFIER
		}
	}

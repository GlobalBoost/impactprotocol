...
use impact_runtime::{
	AccountId, BalancesCall, CheckedExtrinsic, MinimumPeriod,
...
pub struct BlockContentIterator<'a> {
	...
	genesis_hash: impact_primitives::Hash,
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		let signed = self.keyring.sign(
			CheckedExtrinsic {
				signed: Some((
					sender,
					signed_extra(0, impact_runtime::ExistentialDeposit::get() + 1),
				)),
				function: match self.content.block_type {
					BlockType::RandomTransfersKeepAlive =>
						RuntimeCall::Balances(BalancesCall::transfer_keep_alive {
							dest: sp_runtime::MultiAddress::Id(receiver),
							value: impact_runtime::ExistentialDeposit::get() + 1,
						}),
					BlockType::RandomTransfersReaping => {
						RuntimeCall::Balances(BalancesCall::transfer {
							...
							value: 100 * DOLLARS -
								(impact_runtime::ExistentialDeposit::get() - 1),
						})
					},
					BlockType::Noop =>
...
impl BenchKeyring {
	...
	pub fn generate_genesis(&self) -> impact_runtime::GenesisConfig {
		crate::genesis::config_endowed(
			Some(impact_runtime::wasm_binary_unwrap()),

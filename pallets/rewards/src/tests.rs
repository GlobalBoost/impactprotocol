...
fn last_event() -> mock::RuntimeEvent {
...
fn reward_locks_work() {
	new_test_ext(1).execute_with(|| {
		...
		assert_ok!(Balances::transfer(RuntimeOrigin::signed(1), 2, 1));

		// Cannot transfer because of locks
		assert_noop!(
			Balances::transfer(RuntimeOrigin::signed(1), 2, 1),
		...
		// User update locks
		assert_ok!(Rewards::unlock(RuntimeOrigin::signed(1), 1));
		// Locks updated
		expected_locks.remove(&11);
		assert_eq!(Rewards::reward_locks(1), expected_locks);
		// Transfer works
		assert_ok!(Balances::transfer(RuntimeOrigin::signed(1), 2, 10));
		...
		assert_ok!(Balances::transfer(RuntimeOrigin::signed(1), 2, 2));
		assert_noop!(
			Balances::transfer(RuntimeOrigin::signed(1), 2, 1),
			BalancesError::<Test, _>::LiquidityRestrictions
		);
		...
		assert_ok!(Rewards::unlock(RuntimeOrigin::signed(1), 1));
		assert_ok!(Balances::transfer(RuntimeOrigin::signed(1), 2, 22));
		assert_noop!(
			Balances::transfer(RuntimeOrigin::signed(1), 2, 1),
			BalancesError::<Test, _>::LiquidityRestrictions
		);
...
fn curve_works() {
	new_test_ext(1).execute_with(|| {
		...
		assert_eq!(
			last_event(),
			mock::RuntimeEvent::Rewards(crate::Event::<Test>::ScheduleSet)
		);
		...
		assert_eq!(
			last_event(),
			mock::RuntimeEvent::Rewards(crate::Event::<Test>::RewardChanged(100))
		);
		run_to_block(20, 1);
...
fn set_lock_params_works() {
	new_test_ext(1).execute_with(|| {
		// Check initial data
		assert_eq!(Rewards::lock_params(), None);
		// Set lock params
		assert_ok!(Rewards::set_lock_params(
			RuntimeOrigin::root(),
			LockParameters {
		...
		assert_eq!(
			last_event(),
			mock::RuntimeEvent::Rewards(crate::Event::<Test>::LockParamsChanged(LockParameters {
				period: 90,
				divide: 30
			}))
		);

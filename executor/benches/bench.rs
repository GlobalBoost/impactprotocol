...
use impact_primitives::{BlockNumber, Hash};
use impact_testing::keyring::*;
...
pub fn compact_code_unwrap() -> &'static [u8] {
	impact_runtime::WASM_BINARY.expect(
...
const TRANSACTION_VERSION: u32 = impact_runtime::VERSION.transaction_version;

const SPEC_VERSION: u32 = impact_runtime::VERSION.spec_version;
...
fn sign(xt: CheckedExtrinsic) -> UncheckedExtrinsic {
	impact_testing::keyring::sign(xt, SPEC_VERSION, TRANSACTION_VERSION, GENESIS_HASH)
}
...
fn bench_execute_block(c: &mut Criterion) {
	...
	for strategy in execution_methods {
		group.bench_function(format!("{:?}", strategy), |b| {
			let genesis_config = impact_testing::genesis::config(Some(compact_code_unwrap()));
